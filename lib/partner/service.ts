...
export const roomTypePatchSchema = z.object({ name: z.string().min(2).optional(), description: z.string().min(10).max(3000).optional(), totalInventory: z.number().int().min(1).max(500).optional(), maxAdults: z.number().int().min(1).max(20).optional(), maxChildren: z.number().int().min(0).max(20).optional(), amenityIds: z.array(z.string()).max(50).optional() }).strict().refine((value) => Object.keys(value).length > 0, { message: "NOTHING_TO_UPDATE" });
export const ratePlanPatchSchema = z.object({ name: z.string().min(2).optional(), basePricePaise: z.number().int().min(1).optional(), paymentMode: z.enum(["full", "deposit", "pay_at_property"]).optional(), cancellationPolicyId: z.string().min(1).optional() }).strict().refine((value) => Object.keys(value).length > 0, { message: "NOTHING_TO_UPDATE" });
