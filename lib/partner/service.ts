...
export const ratePlanPatchSchema = z.object({ name: z.string().min(2).max(140).optional(), code: z.string().regex(/^[A-Z0-9_-]{2,32}$/).optional(), basePricePaise: z.number().int().min(1).optional(), paymentMode: z.enum(["full", "deposit", "pay_at_property"]).optional(), cancellationPolicyId: z.string().min(1).optional(), status: z.enum(["active", "paused"]).optional() }).strict().refine((value) => Object.keys(value).length > 0, { message: "NOTHING_TO_UPDATE" });
