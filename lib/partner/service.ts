...
export const ratePlanSchema = z.object({ roomTypeId: z.string().min(1), name: z.string().min(2), code: z.string().regex(/^[A-Z0-9_-]{2,32}$/), basePricePaise: z.number().int().min(1), cancellationPolicyId: z.string().min(1), paymentMode: z.enum(["full", "deposit", "pay_at_property"]).default("full"), taxBasisPoints: z.number().int().min(0).max(10000).default(0), customerFeePaise: z.number().int().min(0).default(0), depositBasisPoints: z.number().int().min(0).max(10000).default(2500), minimumNights: z.number().int().min(1).max(365).default(1), maximumNights: z.number().int().min(1).max(365).nullable().default(null) }).strict().refine((value) => value.maximumNights === null || value.maximumNights >= value.minimumNights, { message: "MAXIMUM_NIGHTS_TOO_LOW" });
const bedConfigurationSchema = z.object({ bedType: z.string().min(1).max(60), count: z.number().int().min(1).max(10) });
export const roomTypePatchSchema = z.object({ name: z.string().min(2).max(140).optional(), description: z.string().min(10).max(3000).optional(), totalInventory: z.number().int().min(1).max(500).optional(), maxAdults: z.number().int().min(1).max(20).optional(), maxChildren: z.number().int().min(0).max(20).optional(), maxInfants: z.number().int().min(0).max(10).optional(), bedConfigurations: z.array(bedConfigurationSchema).min(1).max(10).optional(), roomSizeSqFt: z.number().int().min(1).max(10000).nullable().optional(), bathroomType: z.enum(["private", "shared"]).optional(), amenityIds: z.array(z.string()).max(50).optional(), mediaIds: z.array(z.string().min(1)).max(50).optional(), coverMediaId: z.string().min(1).nullable().optional(), status: z.enum(["active", "paused"]).optional() }).strict().refine((value) => Object.keys(value).length > 0, { message: "NOTHING_TO_UPDATE" });
export const ratePlanPatchSchema = z.object({ name: z.string().min(2).max(140).optional(), code: z.string().regex(/^[A-Z0-9_-]{2,32}$/).optional(), basePricePaise: z.number().int().min(1).optional(), paymentMode: z.enum(["full", "deposit", "pay_at_property"]).optional(), cancellationPolicyId: z.string().min(1).optional(), taxBasisPoints: z.number().int().min(0).max(10000).optional(), customerFeePaise: z.number().int().min(0).optional(), depositBasisPoints: z.number().int().min(0).max(10000).optional(), minimumNights: z.number().int().min(1).max(365).optional(), maximumNights: z.number().int().min(1).max(365).nullable().optional(), status: z.enum(["active", "paused"]).optional() }).strict().refine((value) => Object.keys(value).length > 0, { message: "NOTHING_TO_UPDATE" }).refine((value) => !value.minimumNights || value.maximumNights === undefined || value.maximumNights === null || value.maximumNights >= value.minimumNights, { message: "MAXIMUM_NIGHTS_TOO_LOW" });
...
export async function startPropertyDraft(uid: string, name: string, rawOptions: unknown = {}) {
  ...
  const options = startDraftOptionsSchema.parse(rawOptions);
