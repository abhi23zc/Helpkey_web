...
async function dispatch() {
  ...
  for (const document of pending.docs) {
    ...
    catch (error) { await document.ref.set({ dispatchAttempts: FieldValue.increment(1), lastErrorCode: error instanceof Error ? error.name : "DISPATCH_FAILED", updatedAt: FieldValue.serverTimestamp() }, { merge: true }); }
  }
}
