...
export async function GET(request: Request, { params }: RouteContext<"/api/properties/[slug]/bookable">) {
  ...
  const [rooms, rates, policies, amenityCodesById] = await Promise.all([adminDb.collection("roomTypes").where("propertyId", "==", property.id).where("status", "==", "active").get(), adminDb.collection("ratePlans").where("propertyId", "==", property.id).where("status", "==", "active").get(), adminDb.collection("cancellationPolicies").where("propertyId", "==", property.id).where("status", "==", "active").get(), amenityCodeMap()]);
