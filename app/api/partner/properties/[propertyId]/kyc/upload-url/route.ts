...
const rawPOST = async function POST(request: Request, { params }: RouteContext<"/api/partner/properties/[propertyId]/kyc/upload-url">) { const user = await getAuthenticatedUser(); if (!user) return Response.json({ error: "Unauthenticated." }, { status: 401 }); try { const { propertyId } = await params; await propertyOwner(user.uid, propertyId); const input = schema.parse(await request.json()); const uploadId = randomUUID(); const objectKey = `uploads/kyc/${propertyId}/${uploadId}`; const signed = await createR2UploadUrl(objectKey, input.mimeType, input.checksum); await adminDb.collection("pendingUploads").doc(uploadId).set({ propertyId, ownerId: user.uid, kind: "kyc", ...input, objectKey, expiresAt: Date.now() + 15 * 60_000, createdAt: FieldValue.serverTimestamp() }); return Response.json({ uploadId, expiresAt: signed.expiresAt, private: true, uploadUrl: signed.uploadUrl, headers: signed.headers }); } catch (error) { return Response.json({ error: error instanceof Error ? error.message : "Unable to authorize upload." }, { status: 422 }); } }

export const POST = withApiHandler(rawPOST, { route: "/api/partner/properties/[propertyId]/kyc/upload-url", auth: "strict", requireAuth: true, cache: "private" });
