...
export async function POST(request: Request, { params }: RouteContext<"/api/reviews/[propertyId]/photos/upload-url">) {
  ...
  try { const { propertyId } = await params; const input = schema.parse(await request.json()); const reviewId = `${propertyId}_${user.uid}`; const review = await adminDb.collection(REVIEW_COLLECTION).doc(reviewId).get(); if (!review.exists || review.data()?.reviewerId !== user.uid) throw new Error("REVIEW_NOT_FOUND"); const photoCount = Array.isArray(review.data()?.photoIds) ? review.data()?.photoIds.length : 0; if (photoCount >= 5) throw new Error("PHOTO_LIMIT_REACHED"); const uploadId = randomUUID(), objectKey = `reviews/${reviewId}/${uploadId}`; const signed = createR2UploadUrl(objectKey, input.mimeType, input.checksum); await adminDb.collection("pendingUploads").doc(uploadId).set({ reviewId, propertyId, ownerId: user.uid, kind: "review_photo", ...input, objectKey, expiresAt: Date.now() + 15 * 60_000, createdAt: FieldValue.serverTimestamp() }); return Response.json({ uploadId, uploadUrl: signed.uploadUrl, headers: signed.headers, expiresAt: signed.expiresAt, private: true }); }
